...
public :
  ...
  static const int nc = 4;             // Number of RPC chambers.
  static const int np = 3;             // Number of partitions in each chamber.
  bool muonTrigger = true;             // Is the muon trigger being used?
  float trigger_cut_min = 280.0;       // Lower cut to select hits in time with the muon trigger [ns].
  float trigger_cut_max = 380.0;       // Upper cut to select hits in time with the muon trigger [ns].
